...
export async function POST(request: Request) {
  ...
  const result = streamText({
    ...
    tools: {
      getDashboardStats: {
        description:
          "Obtener estadísticas generales del CRM: total cuentas, total clientes, suscripciones activas, por vencer",
        inputSchema: z.object({}),
        execute: () => getDashboardStatsAction(),
      },
      getSubscriptions: {
        description:
          "Listar suscripciones con filtros opcionales por cliente, plataforma o estado",
        inputSchema: z.object({
      ...
      getClientSubscriptions: {
        description:
          "Obtener todas las suscripciones de un cliente por nombre. Retorna cliente con sus datos y todas sus suscripciones agrupadas.",
        inputSchema: z.object({
          nombreCliente: z.string().describe("Nombre del cliente a buscar"),
        }),
        execute: (params) =>
          getClientSubscriptionsAction(params.nombreCliente),
      },
      getClients: {
      ...
      getExpiringSoon: {
        description:
          "Obtener suscripciones que vencen pronto (por defecto 7 días)",
        inputSchema: z.object({
      ...
      getMonthlyRevenue: {
        description:
          "Obtener ingresos mensuales de los últimos 7 meses",
        inputSchema: z.object({}),
      ...
      renewSubscription: {
        description:
          "Renovar una suscripción: resetea fechas a hoy + 1 mes y estado a Activo",
        inputSchema: z.object({
          subscriptionId: z.string().describe("ID de la suscripción a renovar"),
        }),
        execute: (params) =>
          renewSubscriptionAction(params.subscriptionId),
      },
      generateWhatsAppMessage: {
        description:
          "Generar mensaje de WhatsApp con credenciales o renovación para una o más suscripciones. Si son varias del mismo cliente, genera un solo mensaje combinado.",
        inputSchema: z.object({
